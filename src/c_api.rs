...
macro_rules! generate_c_api {
    ($st:ty, $et:ty, $rs_get_flags:ident, $rs_get_offset:ident, $rs_get_size:ident, $rs_get_number_of_sections:ident, $rs_get_section_at:ident, $rs_get_section_name_at:ident, $rs_get_data:ident, $rs_free_exe:ident) => (
        use exe::{Section, Exe};
        ...
        pub extern fn $rs_get_section_name_at<'a>(exe_h: *mut ::libc::c_void, idx: usize) -> *const ::libc::c_char {
            assert_ne!(exe_h, ::std::ptr::null_mut());
            let e = unsafe { Box::from_raw(exe_h as *mut $et) };
            let ret = match e.get_section_name_at(idx) {
                Some(s) => s.as_ptr() as *const ::libc::c_char,
                None => ::std::ptr::null()
            };
            Box::into_raw(e);
            ret
